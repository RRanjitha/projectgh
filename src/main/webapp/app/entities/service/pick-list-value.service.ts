...
export class PickListValueService {
    ...
    getVariety(pickListId: number): Observable<EntityArrayResponseType> {
        return this.http.get<IPickListValue[]>(`${this.resourceUrl}/variety/${pickListId}`, { observe: 'response' });
    }

    getCategory(pickValueId: number): Observable<EntityArrayResponseType> {
        return this.http.get<IPickListValue[]>(`${this.resourceUrl}/category/${pickValueId}`, { observe: 'response' });
    }
}
